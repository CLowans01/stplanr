...
Belfast <- st_read("C:\\Users\\40055486\\Desktop\\NI SOA & OD Files for R\\FermanaghandOmagh2.shp")
...
# extract from od_inter only those codes that are present in Belfast shp file - nb this ought to be redundant due to the input data but is done anyway to play it safe
OD_Omagh <- od_inter %>%
...
desire_short = dplyr::filter(desire_lines, distance < 1000)
# visualise on map
...
# repreats the steps above but for all flow data, NB time intensive
